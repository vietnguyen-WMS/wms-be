...
-- 4.1) NEW: Table user_info (1-1 with users) + trigger + view
CREATE TABLE IF NOT EXISTS ums.user_info (
  ...
  display_name TEXT GENERATED ALWAYS AS (concat_ws(' ', first_name, last_name)) STORED NOT NULL,
  avatar_url   TEXT,
  bio          TEXT,
  address      TEXT,
...
-- 7) Seed/Upsert admin user with password 'admin@123', role = 'admin'
...
SET password_hash = EXCLUDED.password_hash,
    ...
    updated_at    = NOW();
-- 7.1) Seed/Upsert admin user 'viet' with password '1', role = 'admin'
INSERT INTO ums.users (username, password_hash, status_id, role_id, created_at, updated_at)
VALUES (
  'viet',
  crypt('1', gen_salt('bf')),
